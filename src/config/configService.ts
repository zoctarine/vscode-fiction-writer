...
export class ConfigService extends Observable<Config> {
  ...
  reload(event?: ConfigurationChangeEvent) {
    ...
    const dialoguePrefix =
      DialogueMarkerMappings[
        this.read<string>(editDialogue, 'marker', Constants.Dialogue.TWODASH)
      ] ?? '';
    ...
    config.inverseEnter =
      this.read<string>(editing, 'easyParagraphCreation', Constants.Paragraph.NEW_ON_ENTER) ===
    ...
    config.dialogueIndentAutoDetect = this.read<boolean>(
      editDialogue,
      'sentenceIndentAutoDetect',
      true
    );
    ...
    config.compileOutputFormat = this.read<string>(
      exporting,
      'outputFormat.default',
      OutputFormats['odt']
    );
    config.compileShowFormatPicker = this.read<boolean>(
      exporting,
      'outputFormat.alwaysShowFormatPicker',
      true
    );
    config.compileEmDash = this.read<boolean>(exporting, 'smartDeshes', true);
    config.compileShowSaveDialogue =
      this.read<string>(exporting, 'showSaveDialogue', Constants.Compile.SaveDialogue.ALWAYS) ===
    ...
    config.compileShowsErrorInOutputFile = this.read<boolean>(
      exporting,
      'include.showsErrorInOutputFile',
      true
    );
    config.compileIncludeIsEnabled = this.read<boolean>(exporting, 'include.enabled', true);
    config.compileSearchDocumentIdsInAllOpened = this.read<boolean>(
      exporting,
      'include.searchDocumentIdsInAllOpenFilesAndWorkspaces',
      false
    );
    ...
    config.formattingFixMismatchDialogueMarkers = this.read<boolean>(
      formatting,
      'fixMismatchDialogueMarkers',
      true
    );
    config.formattingFixDialogueIndents = this.read<boolean>(
      formatting,
      'fixDialogueIndents',
      true
    );
    config.formattingFixParagraphSpacing = this.read<boolean>(
      formatting,
      'fixParagraphSpacing',
      true
    );
    config.formattingFixParagraphBreaks = this.read<string>(
      formatting,
      'fixParagraphBreaks',
      Constants.Format.ParagraphBreaks.NONE
    );
    ...
    config.formattingRemoveTrailingSpaces = this.read<boolean>(
      formatting,
    ...
    config.viewDialogueHighlightMarkers = this.read<boolean>(
      view,
      'highlight.dialogueMarkers',
      true
    );
    ...
    config.viewFocusModeType = this.read<string>(
      view,
      'focusMode.highlightType',
      Constants.FocusType.PARAGRAPH
    );
    ...
    config.smartRenameRelated = this.read<string>(
      smartRename,
      'renameRelatedFiles',
      Constants.RenameRelated.ASK
    );

    // SPLIT DOCUMENT

    config.splitDocumentSwitchToFileEnabled = this.read<boolean>(
      splitDocument,
      'switchToNewlyCreatedDocument',
      true
    );
    ...
    config.metaKeywordShowInFileExplorer =
      this.read<boolean>(metadata, 'keywords.colorsInFileExplorer', true) && config.metaEnabled;
    config.metaKeywordShowInMetadataView =
      this.read<boolean>(metadata, 'keywords.colorsInMetadataView', true) && config.metaEnabled;

    config.metaCategories = new Map<string, string>();
    config.metaCategoryIconsEnabled =
      this.read<boolean>(metadata, 'categories.showIcons', true) && config.metaEnabled;
    config.metaCategoryNamesEnabled =
      this.read<boolean>(metadata, 'categories.showNames', true) && config.metaEnabled;
    config.metaSummaryCategoryEnabled =
      this.read<boolean>(metadata, 'categories.summaryEnabled', true) && config.metaEnabled;
    ...
    config.metaKeywordBadgeCategory = this.read<string>(
      metadata,
      'keywords.badgesCategory',
      'tags'
    );
    config.metaKeywordColorCategory = this.read<string>(
      metadata,
      'keywords.colorsCategory',
      'tags'
    );
    config.metaFileBadges = new Map<string, string>();
    config.metaKeywordsShowBadges =
      this.read<boolean>(metadata, 'keywords.badgesInFileExplorer', true) && config.metaEnabled;

    const metaKeywordColors = this.read<IKvp<KnownColor>>(metadata, 'keywords.colors', {});
    ...
    const metaFileBadges = this.read<IKvp<string>>(metadata, 'keywords.badges', {});
    ...
    const metaCatIcons = this.read<IKvp<string>>(metadata, 'categories.icons', {});
  ...
  setLocal<T extends string | number | boolean | undefined | { [key: string]: string }>(
    key: string,
    value: T
  ) {
    if (!this._config) {
  ...
  setFlag(key: string) {
    this._localSettings.setValue<boolean>(key, true);
  }
  unsetFlag(key: string) {
    this._localSettings.setValue<boolean>(key, false);
  }

  backup(config: string, key: string): any {
    const value = workspace.getConfiguration(config).get(key);
    this._localSettings.setValue(`${config}.${key}`, value);

    return value;
  }

  restore(config: string, key: string) {
    const storedValue = this._localSettings.getValue(`${config}.${key}`, undefined);

    if (storedValue) {
      workspace.getConfiguration(config).update(key, storedValue);
    }
  }
}
